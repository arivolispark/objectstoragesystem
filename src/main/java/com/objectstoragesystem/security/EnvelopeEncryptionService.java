package com.objectstoragesystem.security;
...
public class EnvelopeEncryptionService {
    private static String clientMasterKeyId = null;
     ...
     */
    public EnvelopeEncryptionService(final AWSCredentials awsCredentials, final String regionEndpointUrl, final String clientMasterKeyId) {
        this.clientMasterKeyId = clientMasterKeyId;
        awskmsClient = new AWSKMSClient(awsCredentials);
        awskmsClient.setEndpoint(regionEndpointUrl);
    }
    ...
    private EnvelopeEncryptedMessage encryptMessage(final String message, final GenerateDataKeyResult dataKey) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException {
        SecretKeySpec key = new SecretKeySpec(dataKey.getPlaintext().array(), Constants.KEY_GENERATOR_ALGORITHM_AES);
        Cipher cipher = Cipher.getInstance(Constants.KEY_GENERATOR_ALGORITHM_AES);
    ...
    public EnvelopeEncryptedMessage encrypt(final String message) {
        try {
            ...
            ByteBuffer ciphertextBlobByteBuffer = keyResult.getCiphertextBlob();
            ByteBuffer plaintextBlobByteBuffer = keyResult.getPlaintext();
    ...
    private SecretKeySpec decryptKey(final EnvelopeEncryptedMessage envelope) {
        ...
        SecretKeySpec key = new SecretKeySpec(plainTextKey.array(), Constants.KEY_GENERATOR_ALGORITHM_AES);
        return key;
    }
